...
} from './declaration-stable-card.js'
import {
...
export class ModeDeclarationStableHost extends LitElement {
  ...
  #renderReactTree(): void {
    ...
    this.#reactRoot.render(
      reactJsx`<${DeclarationStableCard} label="Shadow DOM" testId=${MODE_DECL_STABLE_SHADOW_TEST_ID} />`,
    )
  }
...
export function ensureModeDeclarationStableHostDefined(): void {
  if (!customElements.get(MODE_DECL_STABLE_HOST_TAG)) {
    customElements.define(MODE_DECL_STABLE_HOST_TAG, ModeDeclarationStableHost)
  }
}
